...
class Analyse():
    ...
    def __init__(self):
        ...
        self.path = self.file + '\\analysis'
        isExists = os.path.exists(self.path)  # 判断当前目录是否有文件夹analysis，如果没有则创建
        if not isExists:
            os.mkdir(self.path)  # 在当前路径创建新文件夹analisis,用于存放生成的图像数据
        self.page = Page()  # 创建一个分页用于放所有的图像在这一分页上

    def analyse_Chinaeveryday(self):
        Chian_day=pd.DataFrame(pd.read_excel(self.file+'\\data\\中国总体历史疫情信息\\历史每日新增信息.xlsx'))#读取信息
        ...
        line_CNday_rate.render(self.path+'\\CNdayrate.html')#将生成的图像保存为html文件并存放在analisis文件夹
        line_CNday.render(self.path+'\\Chinaeveryday.html')#将生成的图像保存为html文件并存放在analisis文件夹
        self.page.add(line_CNday)#将图像加入同一页
        self.page.add(line_CNday_rate)#将图像加入同一页
        """
    ...
    def analyse_province(self):
        Province=pd.DataFrame(pd.read_excel(self.file+'\\data\\中国各省市(区)总体疫情信息\\中国各省市(区)总体疫情信息.xlsx'))
        ...
        (
            ...
            .set_global_opts(title_opts=opts.TitleOpts(title='中国各省市(区)总体疫情信息',pos_left='37%',pos_top='5%'),
                             tooltip_opts=opts.TooltipOpts(axis_pointer_type= 'cross'),
                             xaxis_opts=opts.AxisOpts(name='日期'),
                             yaxis_opts=opts.AxisOpts(name='人数'))
        ...
        line_prov_rate.render(self.path+'\\prov_rate.html')#将生成的图像保存为html文件并存放在analisis文件夹
        line_prov.render(self.path+'\\province.html')#将生成的图像保存为html文件并存放在analisis文件夹
        self.page.add(line_prov)#将图像加入同一页
        self.page.add(line_prov_rate)#将图像加入同一页
    ...
    def analyse_Chinahistory(self):
        China_history=pd.DataFrame(pd.read_excel(self.file+'\\data\\中国总体历史疫情信息\\历史总体信息.xlsx'))
        ...
        line_CNhis.render(self.path+'\\Chinahistory.html')#将生成的图像保存为html文件并存放在analisis文件夹
        line_CNhis_rate.render(self.path+'\\Chinahistoryrate.html')#将生成的图像保存为html文件并存放在analisis文件夹
        self.page.add(line_CNhis)#将图像加入同一页
        self.page.add(line_CNhis_rate)#将图像加入同一页
        """
    ...
    def renderall(self):
        self.page.render(self.path+'\\analyse_all.html')#生成包含所有图像的html并存放在analysis文件夹
    
    def main(self):
        self.analyse_Chinaeveryday()#生成中国每日新增信息的折线图
        self.analyse_province()#生成中国各省份信息折线图
        self.analyse_worldhistory()#生成世界总体信息折线图
        self.analyse_Chinahistory()#生成中国总体信息折线图
        self.renderall()

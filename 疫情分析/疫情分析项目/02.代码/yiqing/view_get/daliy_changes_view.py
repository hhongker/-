...
def china_history_all():
    #file = os.path.dirname(os.getcwd())  # 读取上一级文件路径
    file = os.getcwd()  # 读取当前文件路径
    path = file + '\\daliy_changes_view'
    isExists = os.path.exists(path)  # 判断当前目录是否有文件夹photos，如果没有则创建
    if not isExists:
        os.mkdir(path)  # 创建photos,用于存放生成的图像数据

    China_all = pd.DataFrame(pd.read_excel(file+'\\data\\中国总体历史疫情信息\\历史总体信息.xlsx'))  # 读取信息
    ...
    line_CNday.render(file + '\\daliy_changes_view\\中国疫情历史总体信息.html')  # 将生成的图像保存为html文件并存放在photo文件夹
####################中国各省境外输入汇总###########################
def china_provinces_imports():
    ...
    for i in Filelist:
        data1 = pd.read_excel(file + '\\data\\中国各省市(区)疫情信息\\%s' % (i), index=False)
        n = i[:-5]
    ...
    bar.render(file + '\\daliy_changes_view\\中国各省境外输入信息.html', index=False)
##############中国各省市历史疫情信息############
def china_provinces_history():
    ...
    path2 = file + '\\daliy_changes_view\\中国各省市(区)历史疫情信息'
    isExists = os.path.exists(path2)  # 判断当前目录是否有文件夹photos，如果没有则创建
    if not isExists:
        os.mkdir(path2)  # 创建photos,用于存放生成的图像数据

    path = file + '\\data\\中国各省市(区)历史疫情信息'
    Filelist = []
    for home, dirs, files in os.walk(path):
        for filename in files:
            Filelist.append(filename)
    ...
    for i in Filelist:
        data = pd.read_excel(file + '\\data\\中国各省市(区)历史疫情信息\\%s' % (i), index=False)
        n = i[:-5]
        ...
        lines.render(file + '\\daliy_changes_view\\中国各省市(区)历史疫情信息\\%s.html' % (n), index=False)
##############中国各省的城市历史疫情信息#############
def china_citys_history():
    file = os.getcwd()  # 读取当前文件路径

    path = file + '\\data\\中国各省的城市历史疫情信息'
    ...
    for name in floder:
        for root, dirs, files in os.walk(name, topdown=False):
            for filename in files:
                ...
                lines.render_notebook()
                name1 = name.replace('data\\中国各省的城市历史疫情信息', 'daliy_changes_view\\中国各省的城市历史疫情信息')
                name2 = name1.replace('\\', '/')
                name2 = name1.replace('\\', '/')
                save_path = name2 + '/'
                if not os.path.exists(save_path):
                    # print(a)
                    os.makedirs(save_path)  # 在当前路径创建新文件夹,用于存放生成的图像数据
                save_path = save_path + '%s.html'
...
def china_data_all():
    ...
    Chian_day = pd.DataFrame(pd.read_excel('.\\data\\中国总体历史疫情信息\\历史每日新增信息.xlsx'))  # 读取信息
    ...
    line_CNday_rate.render(file + '\\daliy_changes_view\\中国每日治疗率.html')  # 将生成的图像保存为html文件并存放在photo文件夹
    line_CNday.render(file + '\\daliy_changes_view\\中国每日新增信息.html')  # 将生成的图像保存为html文件并存放在photo文件夹
#############各国历史疫情信息#######
def foregin_history_all():
    file = os.getcwd()  # 读取当前文件路径

    path = file + '\\data\\各国历史疫情信息'
    path1 = file + '\\daliy_changes_view'
    isExists = os.path.exists(path1)  # 判断当前目录是否有文件夹如果没有则创建
    if not isExists:
        os.mkdir(path1)  # 用于存放生成的图像数据

    path2 = file + '\\daliy_changes_view\\各国历史疫情信息'
    isExists = os.path.exists(path2)  # 判断当前目录是否有文件夹各国历史疫情信息，如果没有则创建
    if not isExists:
        os.mkdir(path2)  # 用于存放生成的图像数据
    ...
    for i in Filelist:
        data = pd.read_excel(file + '\\data\\各国历史疫情信息\\%s' % (i), index=False)
        ...
        lines.render_notebook()
        lines.render(file + '\\daliy_changes_view\\各国历史疫情信息\\%s.html' % (n), index=False)
#############各国各地区疫情信息#############
def foregin_city_all():
    ...
    for i in Filelist:
        ...
        page.render(file + '\\daliy_changes_view\\各国疫情严重程度排名前十地区信息\\%s.html' % (n), index=False)
#################整体疫情信息#################
def world_data_all():
    ###############################################
    file = os.getcwd()  # 读取当前文件路径
    ...
    world_message = pd.DataFrame(pd.read_excel('.\\data\\世界总体疫情信息\\世界总体疫情信息.xlsx'))  # 获取当前工作路径下的data数据
    ...
    Pie_continent_confirm.render(file + '\\daliy_changes_view\\各大洲确诊信息.html')
    Pie_continent_heal.render(file + '\\daliy_changes_view\\各大洲治愈信息.html')
    Pie_continent_nowconfirm.render(file + '\\daliy_changes_view\\各大洲现存确诊信息.html')
    Line_World.render(file + '\\daliy_changes_view\\各大洲总信息.html')
############疫情严重程度排名前十国家############
def Top10_confirm_country():
    ...
    if not isExists:
        os.mkdir(path1)  # 创建photos,用于存放生成的图像数据
    message = pd.DataFrame(pd.read_excel('.\\data\\世界总体疫情信息\\世界总体疫情信息.xlsx'))  # 获取当前工作路径下的data数据
    ...
    Line_World.render(file + '\\daliy_changes_view\\疫情严重程度排名前十国家（折线图）.html')
    Bar_World.render(file + '\\daliy_changes_view\\疫情严重程度排名前十国家（柱状图）.html')
